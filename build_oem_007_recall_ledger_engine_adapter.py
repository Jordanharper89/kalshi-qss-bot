...
ADAPTER_PATH = INTEGRATION / "oem_007_recall_ledger_engine_adapter.py"
TEST_PATH = ROOT / "test_oem_007_recall_ledger_engine_adapter.py"
INIT_PATH = INTEGRATION / "__init__.py"

ADAPTER_CODE = r'''"""
OEM-007 Recall Ledger Engine Adapter

Canonical Oracle adapter for the existing Oracle Intelligence Recall Ledger Engine.
...
ENGINE_NAME = "Recall Ledger Engine Adapter"
ENGINE_VERSION = "OEM-007"
...
class RecallLedgerEngineAdapter(OracleIntelligenceEngineContract):
    engine_id = ENGINE_ID
    name = ENGINE_NAME
    ...
    def predict(self, payload: Optional[Dict[str, Any]] = None, **kwargs: Any) -> List[Any]:
        ...
        for signal in result.signals:
            ...
            edge = _safe_float(signal.get("edge"), _safe_float(signal.get("recall_score"), 0.0))
            ...
            reason = str(signal.get("reason") or "Recall ledger signal converted through OEM-007 canonical adapter.")
            ...
            predictions.append(
                self._make_prediction(
                    ...
                    metadata={
                        ...
                        "version": self.version,
                        "read_only": True,
                        "recall_type": signal.get("recall_type"),
                        "ledger_key": signal.get("ledger_key"),
    ...
    def _build_underlying_engine(self, runtime_paths: Optional[Any]) -> Optional[Any]:
        candidates = [
            ("qseries_v2.oracle_intelligence.recall_ledger_engine", "RecallLedgerEngine"),
            ("qseries_v2.oracle_intelligence.oracle_recall_ledger_engine", "OracleRecallLedgerEngine"),
            ("qseries_v2.oracle_intelligence.market_recall_ledger_engine", "MarketRecallLedgerEngine"),
        ]
    ...
    def _run_underlying_engine(self, payload: Dict[str, Any]) -> Any:
        ...
        for method_name in ("analyze", "run", "scan", "evaluate", "recall", "query", "resolve"):
    ...
    def _normalize_signals(self, payload: Dict[str, Any], raw_output: Any) -> List[Dict[str, Any]]:
        ...
        for item in raw_signals:
            ...
            score = _safe_float(
                ...
                _safe_float(item.get("ledger_score"), _safe_float(item.get("score"), _safe_float(item.get("edge"), 0.0))),
            )
            ...
            normalized.append(
                {
                    ...
                    "engine_id": self.engine_id,
                    "market_id": market_id,
                    "ledger_key": item.get("ledger_key") or item.get("key") or item.get("pattern_id"),
                    "recall_type": item.get("recall_type") or item.get("type") or "recall_ledger",
                    "recall_score": score,
                    "edge": _safe_float(item.get("edge"), score),
                    "confidence": max(0.0, min(1.0, _safe_float(confidence, 0.5))),
                    "sample_size": item.get("sample_size") or item.get("count") or item.get("matches"),
                    "side": str(item.get("side") or self._side_from_edge(score)).upper(),
                    "reason": item.get("reason")
                    or item.get("explanation")
                    or f"Recall ledger signal detected for {market_id}.",
                    "raw": item,
                    "created_at": _utc_now(),
                }
            )
    ...
    def _extract_raw_signals(self, raw_output: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(raw_output, dict):
            for key in ("signals", "recalls", "ledger", "ledger_entries", "results", "items", "data"):
    ...
    def _signals_from_payload(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        entries = payload.get("entries") or payload.get("markets")
            ...
            return [
                {
                    ...
                    "ledger_key": item.get("ledger_key") or item.get("key") or item.get("pattern_id"),
                    "recall_type": item.get("recall_type") or item.get("type"),
                    "recall_score": item.get("recall_score", item.get("ledger_score", item.get("edge", item.get("score", 0.0)))),
                    "confidence": item.get("confidence", 0.5),
                    "sample_size": item.get("sample_size") or item.get("count") or item.get("matches"),
                    "reason": "Fallback payload-derived recall ledger signal.",
                }
                for item in entries
            ...
            return [
                {
                    ...
                    "recall_type": payload.get("recall_type"),
                    "recall_score": payload.get("recall_score", payload.get("ledger_score", payload.get("edge", payload.get("score", 0.0)))),
...
def build_engine(runtime_paths: Optional[Any] = None) -> RecallLedgerEngineAdapter:
    return RecallLedgerEngineAdapter(runtime_paths=runtime_paths)


def register_with_runtime(runtime: Any, runtime_paths: Optional[Any] = None) -> RecallLedgerEngineAdapter:
...
TEST_CODE = r'''from qseries_v2.integration.oem_007_recall_ledger_engine_adapter import (
    ENGINE_ID,
    RecallLedgerEngineAdapter,
    build_engine,
)
...
def update_init():
    ...
    line = (
        "from .oem_007_recall_ledger_engine_adapter import "
        "RecallLedgerEngineAdapter, build_engine as build_recall_ledger_engine_adapter"
    )
...
def main():
    print("=" * 40)
    print(" OEM-007 INSTALLER")
    print(" Recall Ledger Engine Adapter")
    print("=" * 40)
    ...
    print("\n[DONE] OEM-007 installed")
    print("\nRun:")
    print("py test_oem_007_recall_ledger_engine_adapter.py")
