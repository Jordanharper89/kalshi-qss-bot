...
LEDGER_FILE = MODULE_DIR / "prediction_market_discovery_replay_ledger.py"
TEST_FILE = ROOT / "test_odm_008_prediction_market_discovery_replay_ledger.py"
...
Purpose:
- Capture deterministic fingerprints for ODM/OOS-ready discovery packets.
- Support replay comparison.
- Preserve auditability and explainability.
- Never execute trades or mutate opportunity state.
"""
...
class PredictionMarketDiscoveryReplayLedger:
    ...
    def _entry_from_packet(self, packet: Any) -> PredictionMarketReplayLedgerEntry:
        ...
        audit = packet_payload["audit"]

        payload_summary = MappingProxyType(
            {
                "opportunity_type": payload.get("opportunity_type"),
                "source_engine_id": payload.get("source_engine_id"),
                "validation_required": payload.get("validation_required"),
                "ranking_required": payload.get("ranking_required"),
                "registry_required": payload.get("registry_required"),
...
TEST_CODE = r'''from qseries_v2.oracle_intelligence.oracle_discovery_model.prediction_market_discovery_replay_ledger import (
    PredictionMarketDiscoveryReplayLedger,
)
...
def test_odm_008_prediction_market_discovery_replay_ledger():
    ...
    assert baseline.schema_version == "ODM-008"
    ...
    assert d["schema_version"] == "ODM-008"
    ...
    print("[PASS] ODM-008 Prediction Market Discovery Replay Ledger")
    print(
        {
...
if "PredictionMarketDiscoveryReplayLedger" not in existing_init:
    INIT_FILE.write_text(existing_init.rstrip() + "\n" + INIT_EXPORT.lstrip(), encoding="utf-8")

print("========================================")
print(" ODM-008 INSTALLER")
print(" Prediction Market Discovery Replay Ledger")
print("========================================")
print(f"[OK] Wrote {LEDGER_FILE}")
print(f"[OK] Wrote {TEST_FILE}")
print(f"[OK] Updated {INIT_FILE}")
print()
print("[DONE] ODM-008 installed")
print()
print("Run:")
print("py test_odm_008_prediction_market_discovery_replay_ledger.py")