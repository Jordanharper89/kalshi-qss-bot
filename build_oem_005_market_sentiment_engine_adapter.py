...
ADAPTER_PATH = INTEGRATION / "oem_005_market_sentiment_engine_adapter.py"
TEST_PATH = ROOT / "test_oem_005_market_sentiment_engine_adapter.py"
INIT_PATH = INTEGRATION / "__init__.py"

ADAPTER_CODE = r'''"""
OEM-005 Market Sentiment Engine Adapter

Canonical Oracle adapter for an existing Market Sentiment Engine.
...
ENGINE_NAME = "Market Sentiment Engine Adapter"
ENGINE_VERSION = "OEM-005"
...
class MarketSentimentEngineAdapter(OracleIntelligenceEngineContract):
    """
    Read-only canonical adapter for market sentiment intelligence.

    This adapter normalizes sentiment output into OracleEngineResult signals and
    CanonicalPrediction objects without executing trades or mutating market state.
    """
    ...
    def predict(self, payload: Optional[Dict[str, Any]] = None, **kwargs: Any) -> List[Any]:
        ...
        for signal in result.signals:
            ...
            edge = _safe_float(signal.get("edge"), _safe_float(signal.get("sentiment_score"), 0.0))
            ...
            reason = str(
                ...
                or "Market sentiment signal converted through OEM-005 canonical adapter."
            )
    ...
    def _build_underlying_engine(self, runtime_paths: Optional[Any]) -> Optional[Any]:
        candidates = [
            ("qseries_v2.oracle_intelligence.market_sentiment_engine", "MarketSentimentEngine"),
            ("qseries_v2.oracle_intelligence.sentiment_engine", "SentimentEngine"),
            ("qseries_v2.oracle_intelligence.market_sentiment_analysis_engine", "MarketSentimentAnalysisEngine"),
        ]
    ...
    def _run_underlying_engine(self, payload: Dict[str, Any]) -> Any:
        ...
        for method_name in ("analyze", "run", "scan", "evaluate", "resolve", "score"):
            ...
            normalized.append(
                {
                    ...
                    "engine_id": self.engine_id,
                    "market_id": market_id,
                    "sentiment_source": item.get("sentiment_source") or item.get("source") or "unknown",
                    "sentiment_label": item.get("sentiment_label") or item.get("label") or self._label_from_score(score),
                    ...
                    or f"Sentiment signal detected for {market_id}.",
                    "raw": item,
                    "created_at": _utc_now(),
                }
            )
    ...
    def _extract_raw_signals(self, raw_output: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(raw_output, dict):
            for key in ("signals", "sentiment", "sentiments", "results", "items", "data"):
            ...
            return [
                {
                    ...
                    "sentiment_source": item.get("sentiment_source") or item.get("source"),
                    "sentiment_score": item.get("sentiment_score", item.get("edge", item.get("score", 0.0))),
                    "confidence": item.get("confidence", 0.5),
                    "reason": "Fallback payload-derived market sentiment signal.",
                }
                for item in markets
            ...
            return [
                {
                    ...
                    "sentiment_source": payload.get("sentiment_source"),
                    "sentiment_score": payload.get("sentiment_score", payload.get("edge", payload.get("score", 0.0))),
...
def build_engine(runtime_paths: Optional[Any] = None) -> MarketSentimentEngineAdapter:
    return MarketSentimentEngineAdapter(runtime_paths=runtime_paths)


def register_with_runtime(runtime: Any, runtime_paths: Optional[Any] = None) -> MarketSentimentEngineAdapter:
...
TEST_CODE = r'''from qseries_v2.integration.oem_005_market_sentiment_engine_adapter import (
    ENGINE_ID,
    MarketSentimentEngineAdapter,
    build_engine,
)
...
def update_init():
    ...
    line = (
        "from .oem_005_market_sentiment_engine_adapter import "
        "MarketSentimentEngineAdapter, build_engine as build_market_sentiment_engine_adapter"
    )
...
def main():
    print("=" * 40)
    print(" OEM-005 INSTALLER")
    print(" Market Sentiment Engine Adapter")
    print("=" * 40)
    ...
    print("\n[DONE] OEM-005 installed")
    print("\nRun:")
    print("py test_oem_005_market_sentiment_engine_adapter.py")
