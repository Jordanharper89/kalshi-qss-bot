...
GATE_FILE = MODULE_DIR / "news_intelligence_subsystem_integration_gate.py"
TEST_FILE = ROOT / "test_nid_009_news_intelligence_subsystem_integration_gate.py"
...
from .news_intelligence_discovery_pipeline_gate import NewsIntelligenceDiscoveryPipelineGate
from .news_intelligence_discovery_registry_bridge import NewsIntelligenceDiscoveryRegistryBridge
...
from .news_intelligence_discovery_replay_ledger import NewsIntelligenceDiscoveryReplayLedger

SCHEMA_VERSION = "NID-009"
GATE_ID = "oracle.discovery.gate.news_intelligence_subsystem_integration"
...
class NewsIntelligenceSubsystemIntegrationReport:
    schema_version: str
    gate_id: str
    status: str
    passed_checks: int
...
class NewsIntelligenceSubsystemIntegrationGate:
    schema_version = SCHEMA_VERSION
    gate_id = GATE_ID
    read_only = True

    def __init__(self, source_name: str = "news_intelligence_subsystem_gate_source", min_news_score: float = 0.55) -> None:
    ...
    def capabilities(self) -> Dict[str, Any]:
        return {
            ...
            "validates": [
                "NID-001 contract",
                "NID-002 source adapter",
                "NID-003.1 discovery engine",
                "NID-004 pipeline gate",
                "NID-005 registry bridge",
                "NID-006 pipeline bridge",
                "NID-007 OOS runtime gate",
                "NID-008 replay ledger",
    ...
    def run(self, raw_records: Sequence[Any] | None = None) -> NewsIntelligenceSubsystemIntegrationReport:
        ...
        request = NewsIntelligenceDiscoveryRequest(
            request_id="nid009.news.intelligence.subsystem.integration",
            family=NewsIntelligenceFamily.MARKET_MOVING_NEWS,
            source_name=self.source_name,
            article_ids=tuple(sorted({s.article_id for s in batch.snapshots})),
        ...
        pipeline_gate = NewsIntelligenceDiscoveryPipelineGate(
            source_name=self.source_name,
        ...
        pipeline_bridge = NewsIntelligenceDiscoveryPipelineBridge()
        pipeline_report = pipeline_bridge.bridge_records(registry_report.records)

        oos_gate = NewsIntelligenceDiscoveryOOSRuntimeGate(
            source_name=self.source_name,
            min_news_score=self.min_news_score,
        )
        oos_report = oos_gate.run(raw)

        replay_ledger = NewsIntelligenceDiscoveryReplayLedger()
        ...
        checks = {
            ...
            "contract_schema_ok": CONTRACT_SCHEMA_VERSION == "NID-001",
            ...
            "adapter_schema_ok": batch.schema_version == "NID-002",
            "engine_contract_schema_ok": discovery_report.schema_version == "NID-001",
            "engine_runtime_schema_ok": engine.schema_version == "NID-003.1",
            "pipeline_gate_schema_ok": pipeline_gate_report.schema_version == "NID-004",
            "registry_bridge_schema_ok": registry_report.schema_version == "NID-005",
            "pipeline_bridge_schema_ok": pipeline_report.schema_version == "NID-006",
            "oos_gate_schema_ok": oos_report.schema_version == "NID-007",
            "replay_ledger_schema_ok": replay_a.schema_version == "NID-008",

            "snapshots_emitted": len(batch.snapshots) == len(raw),
            "articles_seen": discovery_report.telemetry.articles_seen == 3,
        ...
        telemetry = MappingProxyType({
            ...
            "articles_seen": discovery_report.telemetry.articles_seen,
        ...
        return NewsIntelligenceSubsystemIntegrationReport(
            schema_version=self.schema_version,
            gate_id=self.gate_id,
...
TEST_CODE = r'''
from qseries_v2.oracle_intelligence.news_intelligence_discovery_model.news_intelligence_subsystem_integration_gate import (
    NewsIntelligenceSubsystemIntegrationGate,
)


def test_nid_009_news_intelligence_subsystem_integration_gate():
    gate = NewsIntelligenceSubsystemIntegrationGate(source_name="nid_009_test_source", min_news_score=0.55)
    ...
    assert report.schema_version == "NID-009"
    assert report.gate_id == "oracle.discovery.gate.news_intelligence_subsystem_integration"
    ...
    print("[PASS] NID-009 News Intelligence Subsystem Integration Gate")
    print({
...
if __name__ == "__main__":
    test_nid_009_news_intelligence_subsystem_integration_gate()
'''
...
if "NewsIntelligenceSubsystemIntegrationGate" not in existing:
    INIT_FILE.write_text(existing.rstrip() + "\n" + INIT_EXPORT.lstrip(), encoding="utf-8")

print("========================================")
print(" NID-009 INSTALLER")
print(" News Intelligence Subsystem Integration Gate")
print("========================================")
print(f"[OK] Wrote {GATE_FILE}")
print(f"[OK] Wrote {TEST_FILE}")
print(f"[OK] Updated {INIT_FILE}")
print()
print("[DONE] NID-009 installed")
print()
print("Run:")
print("py test_nid_009_news_intelligence_subsystem_integration_gate.py")