...
LEDGER_FILE = MODULE_DIR / "macro_event_discovery_replay_ledger.py"
TEST_FILE = ROOT / "test_med_008_macro_event_discovery_replay_ledger.py"
...
SCHEMA_VERSION = "MED-008"
LEDGER_ID = "oracle.discovery.ledger.macro_event_replay"
...
class MacroEventReplayLedgerEntry:
    schema_version: str
    ledger_id: str
    entry_id: str
    packet_id: str
...
@dataclass(frozen=True)
class MacroEventReplayLedgerReport:
    schema_version: str
    ledger_id: str
    status: str
    entries: Tuple[MacroEventReplayLedgerEntry, ...]
    run_fingerprint: str
...
@dataclass(frozen=True)
class MacroEventReplayComparisonReport:
    schema_version: str
    ledger_id: str
    status: str
    matching: bool
...
class MacroEventDiscoveryReplayLedger:
    ...
    def capabilities(self) -> Dict[str, Any]:
        return {
            ...
            "ledger_id": self.ledger_id,
            "read_only": True,
            "accepts": "MED-007 OOS runtime packets",
            "emits": "immutable macro event replay ledger entries",
    ...
    def record_packets(self, packets: Sequence[Any]) -> MacroEventReplayLedgerReport:
        ...
        return MacroEventReplayLedgerReport(
            schema_version=self.schema_version,
            ledger_id=self.ledger_id,
    ...
    def run_discovery_and_record(
        ...
        source_name: str = "macro_event_replay_ledger_source",
        min_event_score: float = 0.55,
    ) -> MacroEventReplayLedgerReport:
        gate = MacroEventDiscoveryOOSRuntimeGate(source_name=source_name, min_event_score=min_event_score)
        return self.record_packets(gate.run(raw_records).packets)

    def compare(self, baseline: MacroEventReplayLedgerReport, replay: MacroEventReplayLedgerReport) -> MacroEventReplayComparisonReport:
        started_at = _utc_now_iso()
        ...
        return MacroEventReplayComparisonReport(
            schema_version=self.schema_version,
            ledger_id=self.ledger_id,
    ...
    def _entry_from_packet(self, packet: Any) -> MacroEventReplayLedgerEntry:
        ...
        packet_payload = {
            ...
            "event_id": getattr(packet, "event_id", None),
        ...
        return MacroEventReplayLedgerEntry(
            schema_version=self.schema_version,
            ledger_id=self.ledger_id,
            ...
            event_id=str(packet_payload["event_id"]),
...
if "MacroEventDiscoveryReplayLedger" not in existing:
    INIT_FILE.write_text(existing.rstrip() + "\n" + INIT_EXPORT.lstrip(), encoding="utf-8")

print("========================================")
print(" MED-008 INSTALLER")
print(" Macro Event Discovery Replay Ledger")
print("========================================")
print(f"[OK] Wrote {LEDGER_FILE}")
print(f"[OK] Wrote {TEST_FILE}")
print(f"[OK] Updated {INIT_FILE}")
print()
print("[DONE] MED-008 installed")
print()
print("Run:")
print("py test_med_008_macro_event_discovery_replay_ledger.py")