...
Purpose:
- Monitor runtime services/jobs.
- Detect unhealthy scheduler, failed services, cache errors, and history errors.
- Restart services through OPS-001 supervisor when allowed.
...
class QSeriesWatchdog:
    ...
    def check_once(self) -> Dict[str, Any]:
        ...
        try:
            supervisor_diag = self.supervisor.diagnostics()

            for service_name, service in supervisor_diag.get("services", {}).items():
                state = service.get("state")
                restart_on_failure = service.get("restart_on_failure")
            ...
            if self.scheduler is not None:
                ...
                if not scheduler_diag.get("scheduler_running"):
                    ...
                    try:
                        ...
                        issues.append({"kind": "scheduler_recovery_failed", "error": str(exc)})

                for job_name, job in scheduler_diag.get("jobs", {}).items():
                    if job.get("error_count", 0) > 0 and job.get("state") == "ERROR":
                        issues.append({
            ...
            result = {
                "module": "ops_005_watchdog_auto_recovery",
                "status": status,
...
if __name__ == "__main__":
    print("OPS-005 Watchdog & Auto-Recovery installed.")
'''

test_code = r'''"""
Test OPS-005 Watchdog & Auto-Recovery.
"""
...
def test_ops_005():
    ...
    # Force service into error state for watchdog recovery test.
    supervisor._services["test.recoverable"].state = "ERROR"
    ...
    print("[PASS] OPS-005 Watchdog & Auto-Recovery")
    print(diag)
...
Path("test_ops_005_watchdog_auto_recovery.py").write_text(test_code, encoding="utf-8")

print("========================================")
print(" OPS-005 INSTALLER")
print(" Watchdog & Auto-Recovery")
print("========================================")
print("[OK] Wrote qseries_v2\\ops\\watchdog.py")
print("[OK] Wrote qseries_v2\\ops\\__init__.py")
print("[OK] Wrote test_ops_005_watchdog_auto_recovery.py")
print()
print("[DONE] OPS-005 installed")
print()
print("Run:")
print("python test_ops_005_watchdog_auto_recovery.py")