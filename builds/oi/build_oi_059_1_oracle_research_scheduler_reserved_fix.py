...
ROOT = Path.cwd()
SCHEDULER = ROOT / "qseries_v2" / "oracle_intelligence" / "oracle_research_scheduler.py"
...
OI-059.1 Fix:
- Prevent duplicate scheduling of already-reserved jobs.
- Only schedule queue_status="queued" items.
- Preserve read-only Oracle boundary.
...
class OracleResearchScheduler:
    ...
    def schedule_next(
        ...
        candidates = self._available_queue_items(min_score=min_score, limit=100000)
        ...
        if ticker in self._scheduled_jobs:
            return None

        if ticker in self.queue_manager._queue:
            self.queue_manager._queue[ticker]["queue_status"] = "reserved"
            self.queue_manager._queue[ticker]["reserved_at"] = self._now()

        scheduled = self._build_schedule_item(item)
        self._scheduled_jobs[ticker] = scheduled
    ...
    def _available_queue_items(self, min_score: float = 0.0, limit: int = 100000) -> List[Dict[str, Any]]:
        items = self.queue_manager.peek(limit=limit, min_score=min_score)
        ...
        for item in items:
            ...
            if item.get("queue_status") not in {None, "queued"}:
                continue
...
SCHEDULER.write_text(scheduler_code, encoding="utf-8")
TEST.write_text(test_code, encoding="utf-8")

print("========================================")
print(" OI-059.1 INSTALLER")
print(" Oracle Research Scheduler Reserved Fix")
print("========================================")
print(f"[OK] Patched {SCHEDULER}")
print(f"[OK] Patched {TEST}")
print()
print("[DONE] OI-059.1 installed")
print()
print("Run:")
