...
ADAPTER_PATH = INTEGRATION / "oem_009_query_resolver_engine_adapter.py"
TEST_PATH = ROOT / "test_oem_009_query_resolver_engine_adapter.py"
INIT_PATH = INTEGRATION / "__init__.py"

ADAPTER_CODE = r'''"""
OEM-009 Query Resolver Engine Adapter

Canonical Oracle adapter for the Universal Market Adapter Query Resolver Engine.
...
ENGINE_NAME = "Query Resolver Engine Adapter"
ENGINE_VERSION = "OEM-009"
...
class QueryResolverEngineAdapter(OracleIntelligenceEngineContract):
    engine_id = ENGINE_ID
    name = ENGINE_NAME
    ...
    def predict(self, payload: Optional[Dict[str, Any]] = None, **kwargs: Any) -> List[Any]:
        ...
        for signal in result.signals:
            market_id = str(
                ...
                or signal.get("resolved_market_id")
                or signal.get("ticker")
                or signal.get("symbol")
                or "unknown_market"
            )

            edge = _safe_float(signal.get("edge"), _safe_float(signal.get("resolver_score"), 0.0))
            ...
            reason = str(signal.get("reason") or "Query resolver signal converted through OEM-009 canonical adapter.")
            ...
            predictions.append(
                self._make_prediction(
                    ...
                    metadata={
                        ...
                        "resolver_type": signal.get("resolver_type"),
                        "resolved_market_id": signal.get("resolved_market_id"),
                        "adapter_id": signal.get("adapter_id"),
                    },
                    created_at=_utc_now(),
                )
            )
    ...
    def _build_underlying_engine(self, runtime_paths: Optional[Any]) -> Optional[Any]:
        candidates = [
            (
                "qseries_v2.oracle_intelligence.universal_market_adapter_query_resolver_engine",
                "UniversalMarketAdapterQueryResolverEngine",
            ),
            ("qseries_v2.oracle_intelligence.query_resolver_engine", "QueryResolverEngine"),
            ("qseries_v2.oracle_intelligence.market_query_resolver_engine", "MarketQueryResolverEngine"),
            ("qseries_v2.oracle_intelligence.oracle_query_resolver_engine", "OracleQueryResolverEngine"),
        ]
    ...
    def _run_underlying_engine(self, payload: Dict[str, Any]) -> Any:
        ...
        for method_name in ("analyze", "run", "scan", "evaluate", "resolve", "resolve_query", "query"):
    ...
    def _normalize_signals(self, payload: Dict[str, Any], raw_output: Any) -> List[Dict[str, Any]]:
        ...
        for item in raw_signals:
            ...
            market_id = str(
                item.get("market_id")
                or item.get("resolved_market_id")
                or item.get("ticker")
                or item.get("symbol")
                or item.get("id")
                or payload.get("market_id")
                or payload.get("resolved_market_id")
                or "unknown_market"
            )

            score = _safe_float(
                item.get("resolver_score"),
                _safe_float(item.get("match_score"), _safe_float(item.get("score"), _safe_float(item.get("edge"), 0.0))),
            )
            ...
            normalized.append(
                {
                    ...
                    "engine_id": self.engine_id,
                    "market_id": market_id,
                    "resolved_market_id": item.get("resolved_market_id") or market_id,
                    "query": item.get("query") or payload.get("query"),
                    "adapter_id": item.get("adapter_id") or item.get("adapter") or payload.get("adapter_id"),
                    "resolver_type": item.get("resolver_type") or item.get("type") or "universal_market_query_resolution",
                    ...
                    or f"Query resolver signal detected for {market_id}.",
                    "raw": item,
                    "created_at": _utc_now(),
                }
            )
    ...
    def _extract_raw_signals(self, raw_output: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(raw_output, dict):
            for key in ("signals", "resolutions", "resolved", "matches", "results", "items", "data", "markets"):
    ...
    def _signals_from_payload(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        items = payload.get("resolutions") or payload.get("matches") or payload.get("markets")

        if isinstance(items, list) and items:
            return [
                {
                    "market_id": item.get("market_id") or item.get("resolved_market_id") or item.get("ticker") or item.get("id"),
                    "resolved_market_id": item.get("resolved_market_id"),
                    "query": item.get("query") or payload.get("query"),
                    "adapter_id": item.get("adapter_id") or item.get("adapter"),
                    "resolver_type": item.get("resolver_type") or item.get("type"),
                    "resolver_score": item.get("resolver_score", item.get("match_score", item.get("edge", item.get("score", 0.0)))),
                    "confidence": item.get("confidence", 0.5),
                    "reason": "Fallback payload-derived query resolver signal.",
                }
                for item in items
                if isinstance(item, dict)
            ]

        if payload.get("market_id") or payload.get("resolved_market_id"):
            return [
                {
                    "market_id": payload.get("market_id") or payload.get("resolved_market_id"),
                    "resolved_market_id": payload.get("resolved_market_id") or payload.get("market_id"),
                    "query": payload.get("query"),
                    "adapter_id": payload.get("adapter_id"),
                    "resolver_type": payload.get("resolver_type"),
                    "resolver_score": payload.get("resolver_score", payload.get("match_score", payload.get("edge", payload.get("score", 0.0)))),
...
def build_engine(runtime_paths: Optional[Any] = None) -> QueryResolverEngineAdapter:
    return QueryResolverEngineAdapter(runtime_paths=runtime_paths)


def register_with_runtime(runtime: Any, runtime_paths: Optional[Any] = None) -> QueryResolverEngineAdapter:
...
TEST_CODE = r'''from qseries_v2.integration.oem_009_query_resolver_engine_adapter import (
    ENGINE_ID,
    QueryResolverEngineAdapter,
    build_engine,
)
...
def update_init():
    ...
    line = (
        "from .oem_009_query_resolver_engine_adapter import "
        "QueryResolverEngineAdapter, build_engine as build_query_resolver_engine_adapter"
    )
...
def main():
    print("=" * 40)
    print(" OEM-009 INSTALLER")
    print(" Query Resolver Engine Adapter")
    print("=" * 40)
    ...
    print("\n[DONE] OEM-009 installed")
    print("\nRun:")
    print("py test_oem_009_query_resolver_engine_adapter.py")
