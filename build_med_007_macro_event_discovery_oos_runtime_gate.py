...
GATE_FILE = MODULE_DIR / "macro_event_discovery_oos_runtime_gate.py"
TEST_FILE = ROOT / "test_med_007_macro_event_discovery_oos_runtime_gate.py"
...
SCHEMA_VERSION = "MED-007"
GATE_ID = "oracle.discovery.gate.macro_event_oos_runtime"
...
class MacroEventOOSRuntimeGateReport:
    schema_version: str
    gate_id: str
    status: str
    passed_checks: int
...
class MacroEventDiscoveryOOSRuntimeGate:
    schema_version = SCHEMA_VERSION
    gate_id = GATE_ID
    read_only = True

    def __init__(self, source_name: str = "macro_event_oos_runtime_gate_source", min_event_score: float = 0.55) -> None:
    ...
    def run(self, raw_records: Sequence[Any] | None = None) -> MacroEventOOSRuntimeGateReport:
        ...
        checks = {
            ...
            "all_packets_have_event_id": all(bool(p.event_id) for p in packets_a),
            ...
            "macro_event_market_shape": all(
                p.payload.get("registry_payload", {}).get("universal_market", {}).get("market_type") == "macro_event"
                for p in packets_a
            ),
            ...
            "source_engine_id_macro_event": all(
                p.payload.get("source_engine_id") == "oracle.discovery.macro_event"
                for p in packets_a
            ),
        ...
        return MacroEventOOSRuntimeGateReport(
            schema_version=self.schema_version,
            gate_id=self.gate_id,
...
if "MacroEventDiscoveryOOSRuntimeGate" not in existing:
    INIT_FILE.write_text(existing.rstrip() + "\n" + INIT_EXPORT.lstrip(), encoding="utf-8")

print("========================================")
print(" MED-007 INSTALLER")
print(" Macro Event Discovery OOS Runtime Gate")
print("========================================")
print(f"[OK] Wrote {GATE_FILE}")
print(f"[OK] Wrote {TEST_FILE}")
print(f"[OK] Updated {INIT_FILE}")
print()
print("[DONE] MED-007 installed")
print()
print("Run:")
print("py test_med_007_macro_event_discovery_oos_runtime_gate.py")