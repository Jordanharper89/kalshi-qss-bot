...
LEDGER_FILE = MODULE_DIR / "crypto_spot_discovery_replay_ledger.py"
TEST_FILE = ROOT / "test_cdm_008_crypto_spot_discovery_replay_ledger.py"
INIT_FILE = MODULE_DIR / "__init__.py"

LEDGER_CODE = r'''"""
CDM-008 Crypto Spot Discovery Replay Ledger

Read-only replay ledger for crypto spot discovery runtime packets.
...
SCHEMA_VERSION = "CDM-008"
LEDGER_ID = "oracle.discovery.ledger.crypto_spot_replay"
LEDGER_NAME = "Crypto Spot Discovery Replay Ledger"
...
class CryptoSpotReplayLedgerEntry:
    schema_version: str
    ledger_id: str
    entry_id: str
    packet_id: str
...
@dataclass(frozen=True)
class CryptoSpotReplayLedgerReport:
    schema_version: str
    ledger_id: str
    status: str
    entries: Tuple[CryptoSpotReplayLedgerEntry, ...]
    run_fingerprint: str
...
@dataclass(frozen=True)
class CryptoSpotReplayComparisonReport:
    schema_version: str
    ledger_id: str
    status: str
    matching: bool
...
class CryptoSpotDiscoveryReplayLedger:
    ...
    def capabilities(self) -> Dict[str, Any]:
        return {
            ...
            "read_only": True,
            "accepts": "CDM-007 OOS runtime packets",
            "emits": "immutable crypto spot replay ledger entries",
    ...
    def record_packets(self, packets: Sequence[Any]) -> CryptoSpotReplayLedgerReport:
        ...
        return CryptoSpotReplayLedgerReport(
            schema_version=self.schema_version,
            ledger_id=self.ledger_id,
    ...
    def run_discovery_and_record(
        ...
        source_name: str = "crypto_spot_replay_ledger_source",
        min_edge_percent: float = 0.01,
        min_liquidity: float = 1000.0,
    ) -> CryptoSpotReplayLedgerReport:
        gate = CryptoSpotDiscoveryOOSRuntimeGate(
    ...
    def compare(
        self,
        baseline: CryptoSpotReplayLedgerReport,
        replay: CryptoSpotReplayLedgerReport,
        ...
        return CryptoSpotReplayComparisonReport(
            schema_version=self.schema_version,
            ledger_id=self.ledger_id,
    ...
    def _entry_from_packet(self, packet: Any) -> CryptoSpotReplayLedgerEntry:
        ...
        return CryptoSpotReplayLedgerEntry(
            schema_version=self.schema_version,
            ledger_id=self.ledger_id,
            entry_id=entry_id,
...
TEST_CODE = r'''from qseries_v2.oracle_intelligence.crypto_discovery_model.crypto_spot_discovery_replay_ledger import (
    CryptoSpotDiscoveryReplayLedger,
)
...
if "CryptoSpotDiscoveryReplayLedger" not in existing_init:
    INIT_FILE.write_text(existing_init.rstrip() + "\n" + INIT_EXPORT.lstrip(), encoding="utf-8")

print("========================================")
print(" CDM-008 INSTALLER")
print(" Crypto Spot Discovery Replay Ledger")
print("========================================")
print(f"[OK] Wrote {LEDGER_FILE}")
print(f"[OK] Wrote {TEST_FILE}")
print(f"[OK] Updated {INIT_FILE}")
print()
print("[DONE] CDM-008 installed")
print()
print("Run:")
print("py test_cdm_008_crypto_spot_discovery_replay_ledger.py")