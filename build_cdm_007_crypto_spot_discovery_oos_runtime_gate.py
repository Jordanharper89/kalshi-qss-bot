...
GATE_FILE = MODULE_DIR / "crypto_spot_discovery_oos_runtime_gate.py"
TEST_FILE = ROOT / "test_cdm_007_crypto_spot_discovery_oos_runtime_gate.py"
...
TEST_CODE = r'''from qseries_v2.oracle_intelligence.crypto_discovery_model.crypto_spot_discovery_oos_runtime_gate import (
    CryptoSpotDiscoveryOOSRuntimeGate,
)


def test_cdm_007_crypto_spot_discovery_oos_runtime_gate():
    gate = CryptoSpotDiscoveryOOSRuntimeGate(
        source_name="cdm_007_test_source",
    ...
    assert report.schema_version == "CDM-007"
    assert report.gate_id == "oracle.discovery.gate.crypto_spot_oos_runtime"
    ...
    assert report.checks["crypto_spot_market_shape"] is True
    assert report.checks["source_engine_id_crypto_spot"] is True
    ...
    d = report.to_dict()
    assert d["schema_version"] == "CDM-007"
    ...
    print("[PASS] CDM-007 Crypto Spot Discovery OOS Runtime Gate")
    print(
        {
...
if __name__ == "__main__":
    test_cdm_007_crypto_spot_discovery_oos_runtime_gate()
'''
...
if "CryptoSpotDiscoveryOOSRuntimeGate" not in existing_init:
    INIT_FILE.write_text(existing_init.rstrip() + "\n" + INIT_EXPORT.lstrip(), encoding="utf-8")

print("========================================")
print(" CDM-007 INSTALLER")
print(" Crypto Spot Discovery OOS Runtime Gate")
print("========================================")
print(f"[OK] Wrote {GATE_FILE}")
print(f"[OK] Wrote {TEST_FILE}")
print(f"[OK] Updated {INIT_FILE}")
print()
print("[DONE] CDM-007 installed")
print()
print("Run:")
print("py test_cdm_007_crypto_spot_discovery_oos_runtime_gate.py")